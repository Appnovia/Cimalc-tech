...
export function ProductForm({ product }: { product?: Product }) {
    ...
    const { register, handleSubmit, formState: { errors } } = useForm<z.input<typeof productSchema>, unknown, ProductInput>({ resolver: zodResolver(productSchema), defaultValues: { name: product?.name ?? "", description: product?.description ?? "", price: product?.price ?? undefined, categorySlug: product?.categorySlug ?? "", inStock: product?.inStock ?? true, images: product?.images ?? [] } });
    ...
    function addImages(event: React.ChangeEvent<HTMLInputElement>) { const files = Array.from(event.target.files ?? []).slice(0, 5 - images.length); const next = files.map((file, index) => ({ id: `${Date.now()}-${index}`, url: URL.createObjectURL(file), alt: file.name.replace(/\.[^/.]+$/, "") })); setImages((current) => [...current, ...next].slice(0, 5)); event.target.value = ""; }
